...
public class DevToxGreanOld extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevToxGreanOld() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 4, 0, 16, 0, 2, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    int iTag = Utility.getShort(bRecv, 3);
                    float testResult = (float)Utility.getShort(bRecv, 5);
                    if (iTag > 0) {
                        testResult = -1.0F * testResult;
                    }

                    this.setParamTestVal((double)testResult, "N", (Timestamp)null);
                    this.setStatus(true);
                    Utility.logInfo("GreanToxOld Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("GreanToxOld Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("GreanToxOld Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            int iResult = this.getCmdResult("GreanToxOld", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevToxGreanOld.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
