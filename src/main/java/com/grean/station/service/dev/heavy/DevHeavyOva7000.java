...
public class DevHeavyOva7000 extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevHeavyOva7000() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{4, 0, 0, 0, 0, 6, (byte)this.mAddress, 4, 0, 1, 0, 16};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 9 && bRecv[6] == this.mAddress && bRecv[7] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("Ova7000Heavy Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("Ova7000Heavy Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("Ova7000Heavy Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                default:
                    ...
                    int iResult = this.getCmdResultTcp("Ova7000Heavy", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevHeavyOva7000.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
