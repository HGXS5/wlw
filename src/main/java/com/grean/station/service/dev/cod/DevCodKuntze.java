...
public class DevCodKuntze extends DevService {
            ...
            if (!this.onCmd) {
                ...
                if (bRecv == null) {
                    if (MonitorService.autoData) {
                        ...
                        Utility.logInfo("KuntzeCOD Dev: 查询失败，通道" + this.mChannel + "，无返回");
                    }
                } else {
                    int iTmp = 0;
                    int i = 0;
                    ...
                    while(true) {
                        if (i >= 4) {
                            dTmp = (double)iTmp / 10.0D;
                            this.setParamTestVal(dTmp, "N", (Timestamp)null);
                            this.setStatus(true);
                            Utility.logInfo("KuntzeCOD Dev: 查询成功，通道" + this.mChannel);
                            break;
                        }

                        if (bRecv[i] < 48 || bRecv[i] > 63) {
                            this.setStatus(false);
                            Utility.logInfo("KuntzeCOD Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                            return;
                        }
        ...
        if (MonitorService.autoData) {
            ...
            switch(this.mCmdType) {
                ...
                default:
                    ...
                    this.onCmd = false;
                    this.saveCmdResult("KuntzeCOD", iResult);
                    return iResult == 1;
                case 8:
                case 9:
                case 10:
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevCodKuntze.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
