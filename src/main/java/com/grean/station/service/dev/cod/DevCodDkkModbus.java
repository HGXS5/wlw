...
public class DevCodDkkModbus extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevCodDkkModbus() {
    }
        ...
        if (!this.onCmd) {
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("DkkCodModbus Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("DkkCodModbus Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("DkkCodModbus Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                default:
                    ...
                    int iResult = this.getCmdResult("DkkCodModbus", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevCodDkkModbus.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
