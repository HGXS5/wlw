...
public class QualityModbus extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public QualityModbus() {
    }
        ...
        if (!this.onCmd) {
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("Modbus Quality: 查询" + queryTag + "成功");
                } else {
                    this.setStatus(false);
                    Utility.logInfo("Modbus Quality: 查询" + queryTag + "失败");
                }
        ...
        if (MonitorService.autoData) {
            ...
            if (bCmd.length > 2) {
                ...
                for(int iTimes = this.mCmdTimes; iTimes > 0; --iTimes) {
                    ...
                    iResult = this.getCmdResult("QualityModbus", bCmd, bRecv);
                    if (iResult == 1) {
                        break;
                    }
                }
