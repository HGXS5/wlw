...
public class DevModbusYuxing extends DevService {
    private Random rand = new Random();
    private boolean onCmd = false;

    public DevModbusYuxing() {
    }
        ...
        if (!this.onCmd) {
            ...
            if (!this.onCmd) {
                ...
                if (!this.onCmd) {
                    ...
                    if (!this.onCmd) {
                        this.getQuery(17, 85, 0, 4, this.mCfgDev.getName() + "扩展参数区");
                    }
                }
            }
        }
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevModbusYuxing.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
