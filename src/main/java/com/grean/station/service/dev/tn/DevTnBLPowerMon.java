...
package com.grean.station.service.dev.tn;
...
public class DevTnBLPowerMon extends DevService {
    ...
    public void init(CommInter interComm, CfgDev cfgDev, CfgFactor cfgFactor, List regList, int channel, int cmdTime) {
        ...
        this.setParam("FactorCode", 21001);
        ...
        if (!this.onCmd) {
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("BLPowerMonTn Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("BLPowerMonTn Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("BLPowerMonTn Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
            ...
            switch(cmdType) {
                ...
                default:
                    ...
                    for(int iTimes = this.mCmdTimes; iTimes > 0; --iTimes) {
                        ...
                        iResult = this.getCmdResult("BLPowerMonTn", bCmd, bRecv);
                        if (iResult == 1) {
                            break;
                        }
                    }
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevTnBLPowerMon.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
