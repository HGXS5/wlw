...
public class DevCodSaihuan extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevCodSaihuan() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 3, -122, 80, 0, 1, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                    float testResult = (float)Utility.getShort(bRecv, 3) / 100.0F;
                    this.setParamTestVal((double)testResult, "N", (Timestamp)null);
                    this.setStatus(true);
                    Utility.logInfo("SaihuanCod Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("SaihuanCod Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("SaihuanCod Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                case 10:
                    return false;
                case 11:
                    bCmd = new byte[]{(byte)this.mAddress, 5, 96, 32, 0, 0, 0, 0};
                    break;
                case 12:
                    return false;
                case 13:
                    return false;
                default:
                    bCmd = new byte[]{(byte)this.mAddress, 5, 96, 32, -1, 0, 0, 0};
            }
            ...
            int iResult = this.getCmdResult("SaihuanCod", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevCodSaihuan.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
