...
public class CentrifugeGrean extends CentrifugeService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public CentrifugeGrean() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 2, 47, 0, 1, 0, 0};
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                case 13:
                    return false;
                default:
                    byte[] bCmd = new byte[]{(byte)this.mAddress, 6, 0, -76, 0, 1, 0, 0};
                    this.onCmd = true;
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                CentrifugeGrean.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
