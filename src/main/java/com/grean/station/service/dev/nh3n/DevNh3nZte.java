...
public class DevNh3nZte extends DevService {
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 16, 0, 0, 12, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("ZteNh3n Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("ZteNh3n Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("ZteNh3n Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                case 8:
                    bCmd = new byte[]{(byte)this.mAddress, 16, 16, -128, 0, 1, 2, 0, 3, 0, 0};
                    break;
                case 9:
                    bCmd = new byte[]{(byte)this.mAddress, 16, 16, -128, 0, 1, 2, 0, 3, 0, 0};
                    break;
                case 10:
                    return false;
                case 11:
                    bCmd = new byte[]{(byte)this.mAddress, 16, 16, -128, 0, 1, 2, 0, 6, 0, 0};
                    break;
                case 12:
                    return false;
                case 13:
                    return false;
                default:
                    bCmd = new byte[]{(byte)this.mAddress, 16, 16, -128, 0, 1, 2, 0, 0, 0, 0};
            }
            ...
            int iResult = this.getCmdResult("ZteNh3n", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevNh3nZte.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
