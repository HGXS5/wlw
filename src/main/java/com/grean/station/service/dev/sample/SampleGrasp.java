...
public class SampleGrasp extends SampleService {
    public SampleGrasp() {
    }

    public void doQuery() {
        byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 0, 8, 0, 3, 0, 0};
        ...
        if (bRecv != null) {
            if (bRecv.length == 11 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                this.setStatus(true);
                Utility.logInfo("Grasp Sample: 查询成功，通道" + this.mChannel);
            } else {
                this.setStatus(false);
                Utility.logInfo("Grasp Sample: 查询失败，通道" + this.mChannel + "，返回格式异常");
            }
        } else if (MonitorService.autoData) {
            this.setStatus(true);
        } else {
            this.setStatus(false);
            Utility.logInfo("Grasp Sample: 查询失败，通道" + this.mChannel + "，无返回");
        }

    }
    @SuppressWarnings("FallThrough")
    public boolean doSample(String strQN) {
        byte[] bSet = new byte[]{(byte)this.mAddress, 16, 0, 8, 0, 3, 6, 0, 0, 1, 44, 0, 1, 0, 0};
        int iResult = 1;
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("Grasp Sample: 留样成功");
                this.cfgSample.setBottle_id(this.sampleVase);
                return true;
            default:
                Utility.logInfo("Grasp Sample: 留样失败");
                --this.sampleVase;
                return false;
        }
    }
