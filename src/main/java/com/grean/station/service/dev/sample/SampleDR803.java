...
public class SampleDR803 extends SampleService {
    public SampleDR803() {
    }

    public void doQuery() {
        byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 0, 5, 0, 1, 0, 0};
        ...
        if (bRecv != null) {
            if (bRecv.length == 7 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                ...
                this.setStatus(true);
                Utility.logInfo("DR803 Sample: 查询成功，通道" + this.mChannel);
            } else {
                this.setStatus(false);
                Utility.logInfo("DR803 Sample: 查询失败，通道" + this.mChannel + "，返回格式异常");
            }
        } else if (MonitorService.autoData) {
            this.setStatus(true);
        } else {
            this.setStatus(false);
            Utility.logInfo("DR803 Sample: 查询失败，通道" + this.mChannel + "，无返回");
        }
    ...
    public boolean doSample(String strQN) {
        byte[] bCmd = new byte[]{(byte)this.mAddress, 6, 0, 1, 0, 1, 0, 0};
        int iResult = 1;
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("DR803 Sample: 留样成功");
                return true;
            default:
                Utility.logInfo("DR803 Sample: 留样失败");
                return false;
        }
    }
