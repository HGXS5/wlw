...
public class DevHeavyOva7000Nc extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevHeavyOva7000Nc() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{0, 0, 0, 0, 0, 6, (byte)this.mAddress, 4, 0, 0, 0, 8};
            ...
            int iIndex;
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 9 && bRecv[6] == this.mAddress && bRecv[7] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("Ova7000HeavyNc Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("Ova7000HeavyNc Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("Ova7000HeavyNc Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        byte[] bCmdStop = new byte[]{0, 0, 0, 0, 0, 6, (byte)this.mAddress, 5, 0, 1, 0, 0};
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                case 13:
                    return false;
                default:
                    byte[] bCmd = new byte[]{0, 0, 0, 0, 0, 6, (byte)this.mAddress, 5, 0, 1, -1, 0};
                    this.onCmd = true;
                    ...
                    int iResult = this.getCmdResultTcp("Ova7000HeavyNc", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevHeavyOva7000Nc.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
