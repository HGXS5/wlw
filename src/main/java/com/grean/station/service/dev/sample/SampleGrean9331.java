...
public class SampleGrean9331 extends SampleService {
    public SampleGrean9331() {
    }
    ...
    public void doQuery() {
        ...
        if (bRecv != null) {
            if (bRecv.length == 4 + 2 * this.vaseSize && bRecv[0] == -52 && bRecv[1] == -35 && bRecv[2] == -86 && bRecv[bRecv.length - 1] == -69) {
                ...
                this.setStatus(true);
                Utility.logInfo("Grean9331 Sample: 查询成功，通道" + this.mChannel);
            } else {
                this.setStatus(false);
                Utility.logInfo("Grean9331 Sample: 查询失败，通道" + this.mChannel + "，返回格式异常");
            }
        } else if (MonitorService.autoData) {
            this.setStatus(true);
        } else {
            this.setStatus(false);
            Utility.logInfo("Grean9331 Sample: 查询失败，通道" + this.mChannel + "，无返回");
        }
    ...
    public boolean doSample(String strQN) {
        ...
        if (bCmd != null) {
            ...
            switch(iResult) {
                case 1:
                    Utility.logInfo("Grean9331 Sample: 留样成功");
                    this.cfgSample.setBottle_id(this.sampleVase);
                    return true;
                default:
                    Utility.logInfo("Grean9331 Sample: 留样失败");
    ...
    public boolean doEmpty(String strQN) {
        ...
        if (bCmd != null) {
            ...
            switch(iResult) {
                case 1:
                    Utility.logInfo("Grean9331 Sample: 排空成功");
                    return true;
                default:
                    Utility.logInfo("Grean9331 Sample: 排空失败");
                    return false;
            }
        } else {
            return false;
    ...
    public boolean doReset() {
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("Grean9331 Sample: 复位成功");
                return true;
            default:
                Utility.logInfo("Grean9331 Sample: 复位失败");
                return false;
        }
    }
}
