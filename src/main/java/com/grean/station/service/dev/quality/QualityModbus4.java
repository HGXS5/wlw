...
public class QualityModbus4 extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public QualityModbus4() {
    }
        ...
        if (!this.onCmd) {
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("Modbus Quality4: 查询" + queryTag + "成功");
                } else {
                    this.setStatus(false);
                    Utility.logInfo("Modbus Quality4: 查询" + queryTag + "失败");
                }
    ...
    public void doQuery() {
        ...
        this.getQuery(48, 0, 0, 16, this.mCfgDev.getName() + "状态区");
        ...
        this.getQuery(64, 0, 0, 12, this.mCfgDev.getName() + "关键参数区");
    }
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                case 5000:
                    ...
                    break;
                case 5001:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 80, 0, 32, 0, 0, 0};
                    break;
                case 5002:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 80, 0, 32, 1, 0, 0};
                    break;
                case 5003:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 80, 0, 32, 2, 0, 0};
                    break;
                case 5004:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 80, 0, 32, 3, 0, 0};
                    break;
                case 5005:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 32, 0, 0, 0};
                    break;
                case 5006:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 32, 1, 0, 0};
                    break;
                case 5007:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 32, 2, 0, 0};
                    break;
                case 5008:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 32, 3, 0, 0};
                    break;
                case 5009:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 32, 4, 0, 0};
                    break;
                case 5010:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 96, 0, 17, 17, 0, 0};
                    break;
                case 6001:
                case 6002:
                case 6003:
            ...
            if (bCmd.length > 2) {
                ...
                for(int iTimes = this.mCmdTimes; iTimes > 0; --iTimes) {
                    ...
                    iResult = this.getCmdResult("QualityModbus4", bCmd, bRecv);
                    if (iResult == 1) {
                        break;
                    }
                }
