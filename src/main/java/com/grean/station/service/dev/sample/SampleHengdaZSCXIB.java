...
public class SampleHengdaZSCXIB extends SampleService {
    public SampleHengdaZSCXIB() {
    }
    ...
    public void doQuery() {
        ...
        if (bRecv != null) {
            if (bRecv.length == 11 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                ...
                int iState = Utility.getShort(bRecv, 5);
                this.setStatus(true);
                Utility.logInfo("HengdaZSCXIB Sample: 查询成功，通道" + this.mChannel);
            } else {
                this.setStatus(false);
                Utility.logInfo("HengdaZSCXIB Sample: 查询失败，通道" + this.mChannel + "，返回格式异常");
            }
        } else if (MonitorService.autoData) {
            this.setStatus(true);
        } else {
            this.setStatus(false);
            Utility.logInfo("HengdaZSCXIB Sample: 查询失败，通道" + this.mChannel + "，无返回");
        }
    ...
    public boolean doSample(String strQN) {
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("HengdaZSCXIB Sample: 启动留样成功");
                return true;
            default:
                Utility.logInfo("HengdaZSCXIB Sample: 启动留样失败");
                return false;
        }
    }
    ...
    public boolean doEmpty(String strQN) {
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("HengdaZSCXIB Sample: 启动排空成功");
                return true;
            default:
                Utility.logInfo("HengdaZSCXIB Sample: 启动排空失败");
                return false;
        }
    }

    public boolean doReset() {
        byte[] bCmd = new byte[]{(byte)this.mAddress, 16, 0, 6, 0, 1, 2, 0, 1, 0, 0};
        this.sampleVase = 0;
        int iResult = 1;
        int crc_result = Utility.calcCrc16(bCmd, 0, bCmd.length - 2);
        byte[] tmp_bytes = Utility.getByteArray(crc_result);
        ...
        switch(iResult) {
            case 1:
                Utility.logInfo("HengdaZSCXIB Sample: 启动复位成功");
                return true;
            default:
                Utility.logInfo("HengdaZSCXIB Sample: 启动复位失败");
                return false;
        }
    }
}
