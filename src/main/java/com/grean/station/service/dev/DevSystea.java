...
public class DevSystea extends DevService {
    ...
    private byte[] getRecv(int waitTime, int recvChannel, boolean bQuery) {
        ...
        while(true) {
            ...
            if (tmpRecv == null) {
                ...
                bLen += tmpRecv.length;
                if (bLen >= 31 && bRecv[bLen - 2] == 13 && bRecv[bLen - 1] == 10) {
                    break;
                }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{35, 68, (byte)this.mAddress, 13, 10};
            ...
            int i;
            if (bRecv != null) {
                if (bRecv.length >= 131 && bRecv[bRecv.length - 2] == 13 && bRecv[bRecv.length - 1] == 10) {
                    String strTmp = new String(bRecv);

                    for(i = 0; i < 4; ++i) {
                        dTmp = (double)Float.parseFloat(strTmp.substring(22 + 9 * i, 30 + 9 * i));
                        this.channelVals[i] = dTmp;
                    }

                    for(i = 4; i < 8; ++i) {
                        dTmp = (double)Float.parseFloat(strTmp.substring(23 + 9 * i, 31 + 9 * i));
                        this.channelVals[i] = dTmp;
                    }

                    for(i = 8; i < 12; ++i) {
                        dTmp = (double)Float.parseFloat(strTmp.substring(24 + 9 * i, 32 + 9 * i));
                        this.channelVals[i] = dTmp;
                    }

                    this.setStatus(true);
                    Utility.logInfo("Systea Dev: 查询成功，通道" + this.mChannel);
                } else {
                ...
                this.setStatus(false);
                Utility.logInfo("Systea Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            this.onCmd = false;
            this.saveCmdResult("Systea", iResult);
            return iResult == 1;
        }
    }
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevSystea.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
