...
public class DevFiveWtw extends DevService {
    ...
    public void doQuery() {
        ...
        byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 0, 0, 0, 40, 0, 0};
        ...
        if (bRecv != null) {
            if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                ...
                this.setStatus(true);
                Utility.logInfo("WtwFive Dev: 查询成功，通道" + this.mChannel);
            } else {
                this.setStatus(false);
                Utility.logInfo("WtwFive Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
            }
            ...
            this.setStatus(false);
            Utility.logInfo("WtwFive Dev: 查询失败，通道" + this.mChannel + "，无返回");
        }
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevFiveWtw.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
