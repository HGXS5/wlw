...
public class DevToxMicroLan extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevToxMicroLan() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 4, 0, 9, 0, 1, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    float testResult = (float)Utility.getShort2(bRecv, 3) / 100.0F;
                    this.setParamTestVal((double)testResult, "N", (Timestamp)null);
                    this.setStatus(true);
                    Utility.logInfo("ToxMicroLan Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("ToxMicroLan Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("ToxMicroLan Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                ...
                case 13:
                    return false;
                default:
                    byte[] bCmd = new byte[]{(byte)this.mAddress, 16, 0, 100, 0, 1, 2, 0, 1, 0, 0};
                    this.onCmd = true;
                    ...
                    for(int iTimes = this.mCmdTimes; iTimes > 0; --iTimes) {
                        ...
                        iResult = this.getCmdResult("ToxMicroLan", bCmd, bRecv);
                        if (iResult == 1) {
                            break;
                        }
                    }
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevToxMicroLan.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
