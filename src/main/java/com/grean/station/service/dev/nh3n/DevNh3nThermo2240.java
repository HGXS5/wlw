...
public class DevNh3nThermo2240 extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevNh3nThermo2240() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 0, 4, 0, 2, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("ThermoNh3n Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("ThermoNh3n Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("ThermoNh3n Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            switch(cmdType) {
                case 8:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 0, 32, 0, 1, 0, 0};
                    break;
                case 9:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 0, 32, 0, 1, 0, 0};
                    break;
                ...
                case 13:
                    return false;
                default:
                    bCmd = new byte[]{(byte)this.mAddress, 6, 0, 30, 0, 1, 0, 0};
            }
            ...
            int iResult = this.getCmdResult("ThermoNh3n", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevNh3nThermo2240.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
