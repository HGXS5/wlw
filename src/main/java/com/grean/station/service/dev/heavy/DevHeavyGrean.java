...
public class DevHeavyGrean extends DevService {
    private boolean onCmd = false;
    private Random rand = new Random();

    public DevHeavyGrean() {
    }
    ...
    public void doQuery() {
        if (!this.onCmd) {
            ...
            byte[] bQuery = new byte[]{(byte)this.mAddress, 4, 16, 1, 0, 8, 0, 0};
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("GreanHeavy Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("GreanHeavy Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("GreanHeavy Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            int iResult = this.getCmdResult("GreanHeavy", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevHeavyGrean.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
