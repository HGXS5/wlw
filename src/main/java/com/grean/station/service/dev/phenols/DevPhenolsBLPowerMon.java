...
package com.grean.station.service.dev.phenols;
...
public class DevPhenolsBLPowerMon extends DevService {
    ...
    public void init(CommInter interComm, CfgDev cfgDev, CfgFactor cfgFactor, List regList, int channel, int cmdTime) {
        ...
        this.setParam("FactorCode", 23002);
        ...
        if (!this.onCmd) {
            ...
            if (bRecv != null) {
                if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 4) {
                    ...
                    this.setStatus(true);
                    Utility.logInfo("BLPowerMonPhenols Dev: 查询成功，通道" + this.mChannel);
                } else {
                    this.setStatus(false);
                    Utility.logInfo("BLPowerMonPhenols Dev: 查询失败，通道" + this.mChannel + "，返回格式异常");
                }
                ...
                this.setStatus(false);
                Utility.logInfo("BLPowerMonPhenols Dev: 查询失败，通道" + this.mChannel + "，无返回");
            }
    ...
    public boolean doDevCmd(int cmdType, String cmdParam, String cmdQN) {
        ...
        if (MonitorService.autoData) {
            ...
            byte[] bCmdStop = new byte[]{(byte)this.mAddress, 5, 0, 0, 0, 0, 0, 0};
            switch(cmdType) {
                case 8:
                    return false;
                case 9:
                ...
                default:
                    ...
                    this.onCmd = true;
                    int crc_result = Utility.calcCrc16(bCmdStop, 0, bCmdStop.length - 2);
                    byte[] tmp_bytes = Utility.getByteArray(crc_result);
                    bCmdStop[bCmdStop.length - 2] = tmp_bytes[2];
                    bCmdStop[bCmdStop.length - 1] = tmp_bytes[3];
                    ...
                    int iResult = this.getCmdResult("BLPowerMonPhenols", bCmd, bRecv);
    ...
    public void doDevCmdThread(final int cmdType, final String cmdParam, final String cmdQN) {
        this.fixedThreadPool.execute(new Runnable() {
            public void run() {
                DevPhenolsBLPowerMon.this.doDevCmd(cmdType, cmdParam, cmdQN);
            }
        });
    }
