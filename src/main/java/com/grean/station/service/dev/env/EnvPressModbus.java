...
public class EnvPressModbus extends DevService {
    private Random rand = new Random();

    public EnvPressModbus() {
    }

    public void doQuery() {
        int regLength = 1;
        this.mAddress = this.mCfgDev.getAddress().intValue();
        byte[] bQuery = new byte[]{(byte)this.mAddress, 3, 0, 0, 0, 1, 0, 0};
        ...
        if (bRecv != null) {
            if (bRecv.length == regLength * 2 + 5 && bRecv[0] == this.mAddress && bRecv[1] == 3) {
                dTmp = (double)Utility.getShort(bRecv, 3) * 1.6D / 2000.0D;
