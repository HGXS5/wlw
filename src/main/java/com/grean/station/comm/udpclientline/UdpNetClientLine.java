...
public class UdpNetClientLine extends CommInter {
  private EventLoopGroup workGroup = new NioEventLoopGroup();
  private UdpHandlerLine myHandler;
  ...
  public UdpNetClientLine(String ip, int port) {
    this.remoteIP = ip;
  ...
  public Bootstrap createBootstrap(Bootstrap bootstrap, EventLoopGroup eventLoop) {
    if (bootstrap != null) {
      final UdpHandlerLine handler = new UdpHandlerLine(this);
      this.myHandler = handler;
      this.myHandler.setRecvListener(new UdpNetClientLine.NetRecvListener());
      bootstrap.group(eventLoop);
      bootstrap.channel(NioDatagramChannel.class);
      bootstrap.handler(new ChannelInitializer<DatagramChannel>() {
        protected void initChannel(DatagramChannel datagramChannel) throws Exception {
          datagramChannel.pipeline().addLast(new ChannelHandler[]{new LineBasedFrameDecoder(512)});
          datagramChannel.pipeline().addLast(new ChannelHandler[]{handler});
        }
      });
      bootstrap.remoteAddress(this.remoteIP, this.remotePort);
      bootstrap.connect().addListener(new UdpListenerLine(this));
    }
  ...
  private class NetRecvListener implements RecvListener {
    ...
    public void OnRecvEvent(byte[] bRecv) {
      if (UdpNetClientLine.this.recvListener != null) {
        UdpNetClientLine.this.recvListener.OnRecvEvent(bRecv);
      }

      synchronized(UdpNetClientLine.this.recvByteLock) {
        if (UdpNetClientLine.this.recvByteList.size() > 40) {
          UdpNetClientLine.this.recvByteList.remove(0);
        }

        UdpNetClientLine.this.recvByteList.add(bRecv);
      }

      String strRecv = Utility.bytesToHexString(bRecv, bRecv.length);
      UdpNetClientLine.this.logger.info("Net Client Recv(" + UdpNetClientLine.this.myHandler.mCtx.channel().remoteAddress() + "): " + strRecv);
    }
  }
}
