...
public class ConnectionListenerModbusTcp implements ChannelFutureListener {
  private CommNetClientModbusTcp client;
  public Logger logger = LoggerFactory.getLogger("net");

  public ConnectionListenerModbusTcp(CommNetClientModbusTcp client) {
    this.client = client;
  }
  ...
  public void operationComplete(ChannelFuture channelFuture) throws Exception {
    if (!channelFuture.isSuccess()) {
      ...
      loop.schedule(new Runnable() {
        public void run() {
          ConnectionListenerModbusTcp.this.client.createBootstrap(new Bootstrap(), loop);
        }
      }, 1L, TimeUnit.SECONDS);
    } else {
