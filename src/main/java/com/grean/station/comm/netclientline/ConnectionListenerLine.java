...
public class ConnectionListenerLine implements ChannelFutureListener {
    private CommNetClientLine client;
    public Logger logger = LoggerFactory.getLogger("net");

    public ConnectionListenerLine(CommNetClientLine client) {
        this.client = client;
    }
    ...
    public void operationComplete(ChannelFuture channelFuture) throws Exception {
        if (!channelFuture.isSuccess()) {
            ...
            loop.schedule(new Runnable() {
                public void run() {
                    ConnectionListenerLine.this.client.createBootstrap(new Bootstrap(), loop);
                }
            }, 3L, TimeUnit.SECONDS);
        } else {
