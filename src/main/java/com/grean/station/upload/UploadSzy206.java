...
public class UploadSzy206 extends UploadInter {
    ...
    public byte[] getHourDataPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            if (!this.bReply) {
                ...
                bStdCheck[iIndex] = Utility.getBCDVal(datetime.getMinuteOfHour());
                bStdCheck[iIndex + 1] = Utility.getBCDVal(datetime.getHourOfDay());
                bStdCheck[iIndex + 2] = Utility.getBCDVal(datetime.getDayOfMonth());
                bStdCheck[iIndex + 3] = Utility.getBCDVal(datetime.getMonthOfYear());
                bStdCheck[iIndex + 4] = Utility.getBCDVal(datetime.getYear());
                iIndex += 5;
            }
            ...
            for(i = 0; i < 40; ++i) {
                ...
                while(var14.hasNext()) {
                    ...
                    if (cfgFactor.getName().equals(((CfgComm206)this.mMonitor.cfgComm206List.get(i)).getName())) {
                        ...
                        while(var16.hasNext()) {
                            RecCheckStd recCheckStd = (RecCheckStd)var16.next();
                            if (recCheckStd.getFactorID() == cfgFactor.getId()) {
                                dTmp = recCheckStd.getTestValue();
                                break;
                            }
                        }
