...
public class Upload212_GD extends UploadInter {
    public Upload212_GD() {
    }
    ...
    public byte[] getBootTimePkg(String strQN, String strFlag) {
        ...
        strSend = strSend + "ST=91;CN=9021;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&&&";
            ...
            while(var5.hasNext()) {
                ...
                if (cfgFactor.getCodeGJ().equals(factorCode)) {
                    ...
                    switch(cmdType) {
                        ...
                        case 4:
                        case 5:
                            this.mMonitor.doDevCmd(cfgFactor.getDevID(), cmdType, "N");
                            break;
                        case 6:
                        case 7:
                            if (strParam != null) {
                                cfgFactor.setRcvrMotherVol(Double.parseDouble(strParam));
                            }

                            this.mMonitor.initScriptFactorList(cfgFactor.getId());
                            this.mMonitor.startTest(cmdType, "N");
                            break;
                        case 8:
        ...
        if (devService.getDevExeListener() == null) {
            devService.setDevExeListener(new DevExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult) {
                    if (cmdQN.length() > 0) {
                        Upload212_GD.this.sendExeResult(cmdQN, cmdID, exeResult);
                    }

                    if (exeResult == 1) {
                        Upload212_GD.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "成功");
                    } else {
                        Upload212_GD.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "失败");
                    }
            ...
            this.mMonitor.sampleService.setSampleExeListener(new SampleExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult, String strDataTime, String strVaseNo) {
                    if (exeResult == 1 && cmdID == 3015) {
                        byte[] bSend = Upload212_GD.this.getRsrvSamplePkg(cmdQN, "8", strDataTime, strVaseNo);
                        Upload212_GD.this.mInterComm.Send(bSend);
                    }
        ...
        if (recDataList == null) {
            ...
            while(true) {
                while(true) {
                    ...
                    } while(!cfgFactor.isParmType() && !cfgFactor.isFiveType() && !cfgFactor.isSysType());
    ...
    public byte[] getHourDataPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                CfgFactor cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isUpload() && (cfgFactor.isParmType() || cfgFactor.isFiveType() || cfgFactor.isSysType())) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecDataFactor)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
    ...
    public byte[] getStdCheckPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            String strSend = "QN=" + strQN + ";";
            strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3016;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
            strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime());
                ...
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isParmType()) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecCheckStd)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
                            strSend = strSend + ";" + cfgFactor.getCodeGJ() + "-CheckValue=" + ((RecCheckStd)recDataList.get(j)).getTestValue() + ",";
    ...
    public byte[] getRcvrCheckPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            String strSend = "QN=" + strQN + ";";
            strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3005;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
            strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime());
                ...
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isParmType()) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecCheckRcvr)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
                            strSend = strSend + ";" + cfgFactor.getCodeGJ() + "-Mixed=" + ((RecCheckRcvr)recDataList.get(j)).getAfterValue() + ",";
                            ...
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Percent=" + ((RecCheckRcvr)recDataList.get(j)).getRcvrRate() + ",";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Flag=ra";
                            break;
                        }
                    }
                }
    ...
    public byte[] getParCheckPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            String strSend = "QN=" + strQN + ";";
            strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3025;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
            strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime());
                ...
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isParmType()) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecCheckPar)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
                            ...
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Rtd=" + ((RecCheckPar)recDataList.get(j)).getValue1st() + ",";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Flag=ps,";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Rtd=" + ((RecCheckPar)recDataList.get(j)).getValue2nd() + ",";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Flag=ps";
                            break;
                        }
                    }
    ...
    public byte[] getLogSysPkg(String strQN, String strFlag, RecDataTime recDataTime) {
        ...
        if (recLogSysList == null) {
            ...
            String strSend = "QN=" + strQN + ";";
            strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=2091;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=1;CP=&&";
            strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime());

            for(int i = 0; i < recLogSysList.size(); ++i) {
                strSend = strSend + ";SYS-Log=" + ((RecLogSys)recLogSysList.get(i)).getLogDesc();
            }
    ...
    public byte[] getLogDevPkg(String strQN, String strFlag, RecDataTime recDataTime, String PolID) {
        ...
        if (recLogDevList == null) {
            ...
            String strSend = "QN=" + strQN + ";";
            strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=2091;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
            strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime());
            int i;
            CfgFactor cfgFactor;
            int j;
                    ...
                    if (cfgFactor.isUsed() && (cfgFactor.isFiveType() || cfgFactor.isParmType())) {
                        for(j = 0; j < recLogDevList.size(); ++j) {
                            if (((RecLogDev)recLogDevList.get(j)).getFactorID() == cfgFactor.getId()) {
                                strSend = strSend + ";" + cfgFactor.getCodeGJ() + "-Log=" + ((RecLogDev)recLogDevList.get(j)).getLogDesc();
                                break;
                            }
                        }
                    }
        ...
        if (InfoID == null) {
            for(j = 0; j < this.mMonitor.devServiceList.size(); ++j) {
                if (cfgFactor.getDevID() == ((DevService)this.mMonitor.devServiceList.get(j)).getDevID()) {
                    ...
                    strTmp = strTmp + ",i13010-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam("MeaProcVal");
                    strTmp = strTmp + ",i13011-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam("LinearFactor");
    ...
    private int checkRecv(byte[] bRecv) {
        String[] cnArray = new String[]{"1000", "1001", "1011", "1012", "1014", "1015", "1061", "1062", "1072", "2011", "2051", "2061", "2062", "2063", "2064", "2065", "2066", "3020", "3016", "3025", "3040", "3041", "3042", "3017", "3044", "3045", "3046", "3047", "3048", "3049", "3050", "3051", "3052", "3053", "3054", "3055", "3056", "3057", "3058", "3059", "3060", "3061", "3062", "3063", "3064", "3065", "3066", "3015", "3018", "3019", "3083", "3084", "3011", "3021"};
                ...
                if (qnRtn > 1) {
                    ...
                    switch(strCN.hashCode()) {
                        ...
                        case 1537406:
                            ...
                            break;
                        case 1537494:
                            if (strCN.equals("2091")) {
                                var17 = 15;
                            }
                            break;
                        case 1567010:
                            if (strCN.equals("3005")) {
                                var17 = 5;
                            }
                            break;
                        case 1567037:
                            if (strCN.equals("3011")) {
                                var17 = 48;
                            }
                            break;
                        case 1567041:
                            ...
                            break;
                        case 1567042:
                            if (strCN.equals("3016")) {
                                var17 = 3;
                            }
                            break;
                        case 1567043:
                            if (strCN.equals("3017")) {
                                var17 = 19;
                            }
                            break;
                        case 1567044:
                            if (strCN.equals("3018")) {
                                var17 = 44;
                            }
                            break;
                        case 1567045:
                            if (strCN.equals("3019")) {
                                var17 = 45;
                            }
                            break;
                        case 1567068:
                    ...
                    switch(var17) {
                        case 0:
                            if (strBeginTime != null && strEndTime != null) {
                                ...
                                if (recTimeList != null) {
                                    var28 = recTimeList.iterator();

                                    while(var28.hasNext()) {
                                        recDataTime = (RecDataTime)var28.next();
                        ...
                        case 43:
                            ...
                            resultList.add(this.getLogResponse(strQN, "收到远程标样测试命令"));
                        ...
                        case 44:
                            ...
                            resultList.add(this.getLogResponse(strQN, "收到远程加标回收命令"));
                        ...
                        case 45:
                            strPolID = this.getCmdValue(bRecv, "PolId");
                            resultList.add(this.getLogResponse(strQN, "收到远程平行样测试命令"));
                        ...
                        case 48:
                            strPolID = this.getCmdValue(bRecv, "PolId");
                            resultList.add(this.getLogResponse(strQN, "收到远程空白校准命令"));
                        ...
                        case 49:
                            strPolID = this.getCmdValue(bRecv, "PolId");
                            resultList.add(this.getLogResponse(strQN, "收到远程标样校准命令"));
    ...
    private byte[] getBeginResponse(String strQN, String strRtn) {
        String strSend = "ST=91;CN=9011;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=0;CP=&&QN=" + strQN + ";QnRtn=" + strRtn + "&&";
        return this.getSendBytes(strSend);
    }

    private byte[] getLogResponse(String strQN, String strLog) {
        String strSend = "QN=" + this.getQN(new Date()) + ";";
        strSend = strSend + "ST=21;CN=2091;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=1;CP=&&DataTime=" + TimeHelper.getCurrentTimeString() + ";NET-Log=" + strLog + "&&";
        return this.getSendBytes(strSend);
    }
    ...
    private String getCmdValue(byte[] bRecv, String cmdName) {
        String strRecv = new String(bRecv);
        int index = strRecv.toUpperCase().indexOf(cmdName.toUpperCase());
        if (index == -1) {
            return null;
        } else {
