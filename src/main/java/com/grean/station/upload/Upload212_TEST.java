...
public class Upload212_TEST extends UploadInter {
    public Upload212_TEST() {
    }
        ...
        if (devService.getDevExeListener() == null) {
            devService.setDevExeListener(new DevExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult) {
                    if (cmdQN.length() > 0) {
                        Upload212_TEST.this.sendExeResult(cmdQN, cmdID, exeResult);
                    }

                    if (exeResult == 1) {
                        Upload212_TEST.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "成功");
                    } else {
                        Upload212_TEST.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "失败");
                    }
            ...
            this.mMonitor.sampleService.setSampleExeListener(new SampleExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult, String strDataTime, String strVaseNo) {
                    if (exeResult == 1 && cmdID == 3015) {
                        byte[] bSend = Upload212_TEST.this.getRsrvSamplePkg(cmdQN, "8", strDataTime, strVaseNo);
                        Upload212_TEST.this.mInterComm.Send(bSend);
                    }
    ...
    private String getDevParam(CfgFactor cfgFactor, String InfoID) {
        ...
        DevService devService;
        if (InfoID == null) {
            var5 = this.mMonitor.devServiceList.iterator();

            while(var5.hasNext()) {
                devService = (DevService)var5.next();
                if (cfgFactor.getDevID() == devService.getDevID()) {
                    if (devService.getClass().equals(DevFlowLs.class)) {
                        strTmp = strTmp + ";PolId=" + cfgFactor.getCodeGJ();
                        strTmp = strTmp + ",i33001-Info=" + devService.getParam(4);
                        strTmp = strTmp + ",i33002-Info=" + devService.getParam(2);
                        strTmp = strTmp + ",i33003-Info=" + devService.getParam(3);
                        strTmp = strTmp + ",i33004-Info=" + devService.getParam(0);
                        strTmp = strTmp + ",i33005-Info=" + devService.getParam(1);
                    } else {
                        strTmp = strTmp + ";PolId=" + cfgFactor.getCodeGJ();
                        strTmp = strTmp + ",i13001-Info=" + devService.getParam("RangeTop");
                        strTmp = strTmp + ",i13002-Info=" + devService.getParam("MeaAccuracy");
                        strTmp = strTmp + ",i13003-Info=" + devService.getParam("MeaGap");
                        strTmp = strTmp + ",i13004-Info=" + devService.getParam("DigTemp");
                        strTmp = strTmp + ",i13005-Info=" + devService.getParam("DigTime");
                        strTmp = strTmp + ",i13006-Info=" + TimeHelper.formatDataTime_yyyyMMddHHmmss((Timestamp)devService.getParam("BlnkCalTime"));
                        ...
                        dTmp = (Double)devService.getParam("DetectionVal");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13009-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("MeaProcVal");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13010-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("LinearFactor");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13011-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("BinomialFactor");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13012-Info=" + Utility.getLongDouble(dTmp);
                        }

                        strTmp = strTmp + ",i13013-Info=" + TimeHelper.formatDataTime_yyyyMMddHHmmss((Timestamp)devService.getParam("StdCalTime"));
                        dTmp = (Double)devService.getParam("DilutionFactor");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13014-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("TrinomialFactor");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13015-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("BlnkCalFactor");
                        if (dTmp != null) {
                            strTmp = strTmp + ",i13016-Info=" + Utility.getLongDouble(dTmp);
                        }

                        dTmp = (Double)devService.getParam("RangeFactor");
                        if (dTmp != null) {
            ...
            while(var5.hasNext()) {
                ...
                if (cfgFactor.getDevID() == devService.getDevID()) {
                    ...
                    if (devService.getClass().equals(DevFlowLs.class)) {
                        ...
                        switch(InfoID.hashCode()) {
                            case -1240239864:
                                if (InfoID.equals("i33001")) {
                                    var8 = 0;
                                }
                                break;
                            case -1240239863:
                                if (InfoID.equals("i33002")) {
                                    var8 = 1;
                                }
                                break;
                            case -1240239862:
                                if (InfoID.equals("i33003")) {
                                    var8 = 2;
                                }
                                break;
                            case -1240239861:
                                if (InfoID.equals("i33004")) {
                                    var8 = 3;
                                }
                                break;
                            case -1240239860:
                                if (InfoID.equals("i33005")) {
                                    var8 = 4;
                                }
                        }
    ...
    private String getDevParamFive(CfgFactor cfgFactor, String InfoID) {
        ...
        if (InfoID == null) {
            for(j = 0; j < this.mMonitor.devServiceList.size(); ++j) {
                if (cfgFactor.getDevID() == ((DevService)this.mMonitor.devServiceList.get(j)).getDevID()) {
                    if (((DevService)this.mMonitor.devServiceList.get(j)).getClass().equals(DevFlowLs.class)) {
                        strTmp = strTmp + ";PolId=" + cfgFactor.getCodeGJ();
                        strTmp = strTmp + ",i33001-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam(4);
                        strTmp = strTmp + ",i33002-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam(2);
                        strTmp = strTmp + ",i33003-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam(3);
                        strTmp = strTmp + ",i33004-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam(0);
                        strTmp = strTmp + ",i33005-Info=" + ((DevService)this.mMonitor.devServiceList.get(j)).getParam(1);
                    } else {
                        strTmp = strTmp + ";PolId=" + cfgFactor.getCodeGJ();
