...
import com.grean.station.utils.CmdUtil;
import com.grean.station.utils.FileDownloadUtil;
import com.grean.station.utils.TimeHelper;
import com.grean.station.utils.Utility;
import com.grean.station.utils.ZipUtil;
...
public class Upload212_HF extends UploadInter {
    public Upload212_HF() {
    }
        ...
        if (devService.getDevExeListener() == null) {
            devService.setDevExeListener(new DevExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult) {
                    if (cmdQN.length() > 0) {
                        Upload212_HF.this.sendExeResult(cmdQN, cmdID, exeResult);
                    }

                    if (exeResult == 1) {
                        Upload212_HF.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "成功");
                    } else {
                        Upload212_HF.this.mMonitor.saveRecLogSys("系统日志", "启动" + devService.getDevName() + "失败");
                    }
            ...
            this.mMonitor.sampleService.setSampleExeListener(new SampleExeListener() {
                public void OnExeEvent(String cmdQN, int cmdID, int exeResult, String strDataTime, String strVaseNo) {
                    if (exeResult == 1 && cmdID == 3015) {
                        byte[] bSend = Upload212_HF.this.getRsrvSamplePkg(cmdQN, "8", strDataTime, strVaseNo);
                        Upload212_HF.this.mInterComm.Send(bSend);
                    }
    ...
    private byte[] getSysEnvironmentPkg(String strQN, String strFlag) {
        ...
        strSend = strSend + "DataTime=" + df.format(new Date());
        CfgStation cfgStation = this.mMonitor.getCfgMapper().getCfgStation();
        Double dTmp = this.getFactorVal("Lng");
        if (dTmp != null) {
            strSend = strSend + ";Lng=" + dTmp;
        } else {
            strSend = strSend + ";Lng=" + cfgStation.getLongitude();
        }

        dTmp = this.getFactorVal("Lat");
        if (dTmp != null) {
            strSend = strSend + ";Lat=" + dTmp;
        } else {
            strSend = strSend + ";Lat=" + cfgStation.getLatitude();
        }

        dTmp = this.getFactorVal("Temp");
        if (dTmp != null) {
            strSend = strSend + ";Temp=" + dTmp;
        }

        dTmp = this.getFactorVal("Hum");
        if (dTmp != null) {
            strSend = strSend + ";Hum=" + dTmp;
        }

        dTmp = this.getFactorVal("Volt");
        if (dTmp != null) {
            strSend = strSend + ";Volt=" + dTmp;
        }
    ...
    private byte[] getStationVersionPkg(String strQN, String strFlag) {
        String strSend = "QN=" + strQN + ";";
        strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3100;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
        strSend = strSend + "Version=V1.0.0.1";
        strSend = strSend + "&&";
        return this.getSendBytes(strSend);
    }

    private byte[] getDeviceVersionPkg(String strQN, String strPolId, String strFlag) {
        String strSend = "QN=" + strQN + ";";
        strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3102;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
        strSend = strSend + "FirmwareVersion=V1.0.0.1";
        strSend = strSend + "&&";
        return this.getSendBytes(strSend);
    }
    ...
    private Double getFactorVal(String factorCode) {
        for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
            if (((CfgFactor)this.mMonitor.cfgFactorList.get(i)).getCode().equalsIgnoreCase(factorCode)) {
    ...
    public byte[] getRealTimePkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                ...
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && (cfgFactor.isFiveType() || cfgFactor.isParmType()) && cfgFactor.getCodeGJ() != null) {
    ...
    public byte[] getHourDataPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                CfgFactor cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isUpload() && (cfgFactor.isParmType() || cfgFactor.isFiveType() || cfgFactor.isSysType() || cfgFactor.isSwType()) && cfgFactor.getCodeGJ() != null) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecDataFactor)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
    ...
    public byte[] getFiveDataPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                CfgFactor cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isFiveType() && cfgFactor.getCodeGJ() != null) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecDataFactor)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
                            strSend = strSend + ";" + cfgFactor.getCodeGJ() + "-Avg=" + ((RecDataFactor)recDataList.get(j)).getDataString() + ",";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Flag=" + ((RecDataFactor)recDataList.get(j)).getDataFlag();
    ...
    public byte[] getStdCheckPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                CfgFactor cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if ((factorID <= 0 || factorID == cfgFactor.getId()) && cfgFactor.isUsed() && cfgFactor.isParmType() && cfgFactor.getCodeGJ() != null) {
                    for(int j = 0; j < recDataList.size(); ++j) {
    ...
    public byte[] getStateDevPkg(String strQN, String strFlag, RecDataTime recDataTime, String PolID, String InfoID) {
        ...
        if (PolID == null) {
            for(i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                ...
                if (cfgFactor.isUsed() && (cfgFactor.isFiveType() || cfgFactor.isParmType()) && cfgFactor.getCodeGJ() != null) {
    ...
    public byte[] getParamDevPkg(String strQN, String strFlag, RecDataTime recDataTime, String PolID, String InfoID) {
        ...
        if (PolID == null) {
            for(i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if (cfgFactor.isUsed() && cfgFactor.getCodeGJ() != null) {
    ...
    private int checkRecv(byte[] bRecv) {
        String[] cnArray = new String[]{"1000", "1001", "1011", "1012", "1014", "1015", "1061", "1062", "1072", "2011", "2051", "2061", "2062", "2063", "2064", "2065", "2066", "3015", "3020", "3021", "3040", "3041", "3042", "3043", "3044", "3045", "3046", "3047", "3048", "3049", "3050", "3051", "3052", "3053", "3054", "3055", "3056", "3057", "3058", "3059", "3060", "3061", "3062", "3063", "3064", "3065", "3066", "3080", "3081", "3082", "3083", "3084", "3085", "3086", "3100", "3101", "3102", "3103", "3200"};
        if (bRecv.length > 26 && bRecv[0] == 35 && bRecv[1] == 35) {
            ...
            if (iLen + 10 == bRecv.length) {
                ...
                if (strRecv.substring(strRecv.length() - 4, strRecv.length()).equalsIgnoreCase(strTmp)) {
    ...
    public List<byte[]> getResponse(byte[] bRecv) {
        int qnRtn;
        int exeRtn = 1;
        final String strQN = this.getCmdValue(bRecv, "QN");
        String strCN = this.getCmdValue(bRecv, "CN");
        if (strQN != null && strCN != null) {
            ...
            if (qnRtn == 0) {
                ...
                switch(strCN.hashCode()) {
                    case 1567966:
                        if (strCN.equals("3100")) {
                            var14 = 0;
                        }
                        break;
                    case 1567968:
                        if (strCN.equals("3102")) {
                            var14 = 1;
                        }
                }
                ...
                switch(var14) {
                    case 0:
                    case 1:
                        resultList.add(this.getBeginResponseWithQN(strQN, qnRtn + ""));
                        break;
                    default:
                ...
                if (qnRtn > 1) {
                    ...
                    RecLogPlatform recLogPlatform = new RecLogPlatform();
                    recLogPlatform.setRecPlatform(this.mUploadCfg.getId());
                    byte var18 = -1;
                    switch(strCN.hashCode()) {
                        case 1507423:
                        ...
                        case 1567968:
                            ...
                            break;
                        case 1567969:
                            if (strCN.equals("3103")) {
                                var18 = 56;
                            }
                            break;
                        case 1568927:
                            if (strCN.equals("3200")) {
                                var18 = 45;
                            }
                    }
                    ...
                    switch(var18) {
                        case 0:
                            if (strBeginTime != null && strEndTime != null) {
                                ...
                                if (recTimeList != null) {
                                    var36 = recTimeList.iterator();

                                    while(var36.hasNext()) {
                                        recDataTime = (RecDataTime)var36.next();
                        ...
                        case 10:
                            ...
                            recLogPlatform.setRecDesc("[提取系统时间]成功");
                            break;
                        case 11:
                            strTmp = this.getCmdValue(bRecv, "SystemTime");
                            Timestamp timestamp = TimeHelper.getDataTime_yyyy_MM_ddHHmmss(strTmp);
                        ...
                        case 16:
                            ...
                            if (strPolIDList.size() > 0 && strInfoIDList.size() > 0 && strPolIDList.size() == strInfoIDList.size()) {
                                label510:
                                for(i = 0; i < strPolIDList.size(); ++i) {
                                    if (((String)strPolIDList.get(i)).equals("w00000")) {
                                        String var40 = ((String)strInfoIDList.get(i)).substring(0, 3);
                                        byte var41 = -1;
                                        ...
                                        String var26 = ((String)strInfoIDList.get(i)).substring(0, 3);
                                        byte var27 = -1;
                        ...
                        case 45:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(1, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 46:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(2, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 47:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(7, strQN, strPolID, strTmp);
                                }
                            });
                            break;
                        case 48:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(6, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 49:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(3, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 50:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(4, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 51:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(8, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 52:
                            ...
                            this.fixedThreadPool.execute(new Runnable() {
                                public void run() {
                                    Upload212_HF.this.setDevCmd(9, strQN, strPolID, (String)null);
                                }
                            });
                            break;
                        case 53:
                            bTmp = this.getStationVersionPkg(strQN, "8");
                            if (bTmp != null) {
                                resultList.add(bTmp);
                            }

                            recLogPlatform.setRecDesc("[获取软件版本号]成功");
                            break;
                        case 54:
                            resultList.add(this.getEndResponse(strQN, exeRtn + ""));

                            for(i = 0; i < resultList.size(); ++i) {
                                this.mInterComm.Send((byte[])resultList.get(i));
                            }

                            strTmp = this.getCmdValue(bRecv, "URL");
                            String timeTag = this.getQN(new Date());
                            FileDownloadUtil.downloadNet(strTmp, "c:/station" + timeTag + ".zip");
                            ZipUtil.unzip("c:\\station" + timeTag + ".zip", "c:\\");
                            recLogPlatform.setRecDesc("[软件升级]成功");

                            try {
                                (new Thread(new Runnable() {
                                    public void run() {
                                        String strCmd = CmdUtil.runConsoleCommand("cmd.exe /c start restart.bat");
                        ...
                        case 55:
                            strPolID = this.getCmdValue(bRecv, "PolId");
                            bTmp = this.getDeviceVersionPkg(strQN, strPolID, "8");
                            if (bTmp != null) {
                                resultList.add(bTmp);
                            }

                            CfgFactor cfgFactor = this.mMonitor.getCfgFactorByGjCode(strPolID);
                            if (cfgFactor == null) {
                                recLogPlatform.setRecDesc("[获取固件版本号]成功");
                            } else {
                                recLogPlatform.setRecDesc("[获取固件版本号]成功 - " + cfgFactor.getName());
                            }
                            break;
                        case 56:
                            strPolID = this.getCmdValue(bRecv, "PolId");
                            strTmp = this.getCmdValue(bRecv, "URL");
                            FileDownloadUtil.downloadNet(strTmp, "c:/update-" + strPolID + ".apk");
                            recLogPlatform.setRecDesc("[固件升级]成功");
                            break;
                        default:
                            exeRtn = 2;
                    }

                    recLogPlatform.setRecResult(exeRtn == 1);
                    if (recLogPlatform.getRecDesc() == null) {
                        recLogPlatform.setRecDesc(strCN);
                    }

                    this.mMonitor.getRecMapper().addRecLogPlatform(recLogPlatform);
                    resultList.add(this.getEndResponse(strQN, exeRtn + ""));
                    return resultList;
                }
            }
    ...
    private byte[] getBeginResponseWithQN(String strQN, String strRtn) {
        String strSend = "QN=" + strQN + ";";
        strSend = strSend + "ST=91;CN=9011;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=8;CP=&&QN=" + strQN + ";QnRtn=" + strRtn + "&&";
        return this.getSendBytes(strSend);
    }
    ...
    public byte[] getFlowDataPkg(String strQN, String strFlag, RecDataTime recDataTime, int factorID) {
        ...
        if (recDataList == null) {
            ...
            for(int i = 0; i < this.mMonitor.cfgFactorList.size(); ++i) {
                CfgFactor cfgFactor = (CfgFactor)this.mMonitor.cfgFactorList.get(i);
                if (cfgFactor.isUsed() && cfgFactor.isSwType() && cfgFactor.getCodeGJ() != null && (factorID <= 0 || factorID == cfgFactor.getId())) {
                    for(int j = 0; j < recDataList.size(); ++j) {
                        if (((RecDataFactor)recDataList.get(j)).getFactorID() == cfgFactor.getId()) {
                            strSend = strSend + ";" + cfgFactor.getCodeGJ() + "-Rtd=" + ((RecDataFactor)recDataList.get(j)).getDataString() + ",";
                            strSend = strSend + cfgFactor.getCodeGJ() + "-Flag=" + ((RecDataFactor)recDataList.get(j)).getDataFlag();
    ...
    public byte[] getSystemAlarmPkg(String strQN, String strFlag, Timestamp alarmTime, String alarmType) {
        String strSend = "QN=" + strQN + ";";
        strSend = strSend + "ST=" + this.mUploadCfg.getSt() + ";CN=3201;PW=" + this.mUploadCfg.getPassword() + ";MN=" + this.mUploadCfg.getMn() + ";Flag=" + strFlag + ";CP=&&";
        strSend = strSend + "DataTime=" + TimeHelper.formatDataTime_yyyyMMddHHmmss(alarmTime);
        strSend = strSend + ";SystemAlarm=" + alarmType;
        strSend = strSend + "&&";
        return this.getSendBytes(strSend);
    }
}
