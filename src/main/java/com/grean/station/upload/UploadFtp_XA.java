...
public class UploadFtp_XA extends UploadInter {
    public UploadFtp_XA() {
    }
                    ...
                    do {
                        do {
                            if (!var7.hasNext()) {
                                ...
                                strSend = strSend + "<?xml version=\"1.0\"?>\r\n";
                                strSend = strSend + "<Body xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\r\n";
                                strSend = strSend + "\t<Head fsfid=\"" + this.mUploadCfg.getMn() + "\" jsfid=\"" + this.mUploadCfg.getMn() + "\" wjscsj=\"" + TimeHelper.formatDataTime_yyyyMMddHHmmss(TimeHelper.getCurrentTimestamp()) + "\" jls=\"" + iCount + "\">\n";
                                strSend = strSend + "\t\t<Data ywlx=\"WQ\" sjcssj=\"" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime()) + "\" sjjssj=\"" + TimeHelper.formatDataTime_yyyyMMddHHmmss(recDataTime.getRecTime()) + "\">\r\n";
                                Iterator var14 = this.mMonitor.cfgFactorList.iterator();
                                ...
                                while(true) {
                                    while(true) {
                                        ...
                                        Iterator var10 = recDataList.iterator();
                                        ...
                                        while(var10.hasNext()) {
                                            ...
                                            if (recDataFactor.getFactorID() == cfgFactor.getId()) {
                                                if (cfgFactor.getCodeGJ() != null && cfgFactor.getCodeGJ().length() > 0) {
                                                    String strStatus = "N";
                                                    if (recDataFactor.getDataFlag().equalsIgnoreCase("N")) {
                                                        if (recDataFactor.getDataValue() < cfgFactor.getAlarmMin()) {
                                                            strStatus = "U";
                                                        }

                                                        if (recDataFactor.getDataValue() > cfgFactor.getAlarmMax()) {
                                                            strStatus = "O";
                                                        }
                                                    }

                                                    if (recDataFactor.getDataFlag().equalsIgnoreCase("D")) {
                                                        strStatus = "D";
                                                    }

                                                    strSend = strSend + "\t\t\t<Item pollutant_code=\"" + cfgFactor.getCodeGJ() + "\" equip_status=\"" + strStatus + "\" value=\"" + recDataFactor.getDataValue() + "\"/>\r\n";
                                                }
                                                break;
                                            }
                                        }
